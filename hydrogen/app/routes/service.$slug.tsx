...
export const meta: Route.MetaFunction = ({data}) => {
  ...
  return [
    {title: `${data.service.name} | DIBI Milano Skin Center`},
    ...
    <div className="min-h-screen bg-[#F9F9F7]">
              ...
              <section key={index} className={`py-16 md:py-24 ${!isEven ? 'bg-[#F9F9F7]' : 'bg-white'}`}>
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                  <div className="text-center mb-16">
                    ...
                    {section.subtitle && (
                      <p className="text-lg font-serif text-gray-600 max-w-2xl mx-auto">{section.subtitle}</p>
                    )}
                  </div>
                    ...
                    {section.items?.map((item, i) => (
                      <div key={i} className="flex flex-col sm:flex-row sm:items-baseline justify-between border-b border-gray-200 pb-4 group">
                        <div className="flex-1 pr-4">
                          ...
                          {item.description && (
                            <p className="text-sm font-serif text-gray-500 mt-1 italic">{item.description}</p>
                          )}
                        </div>
                        <div className="mt-2 sm:mt-0 whitespace-nowrap">
                          <span className="text-lg font-mono text-gray-900 tracking-tight">{item.price}</span>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <section className="py-32 bg-white text-center px-4">
        ...
        <div className="mt-8">
          <Link
            to="/"
            className="text-xs font-medium tracking-widest text-gray-500 hover:text-[#D4C5B9] transition-colors uppercase"
          >
            ← Back to Home
          </Link>
        </div>
